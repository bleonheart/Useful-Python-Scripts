...
def group_files():
    cats = {k: defaultdict(list) for k in ('sound','particle','image','material','model')}
...
def find_unused_non_materials(lua_text: str, cats):
    ...
    for name, paths in cats['sound'].items():
        if name not in lua_text:
            unused.extend(paths); removed['sound'] += len(paths)
    for name, paths in cats['particle'].items():
        if name not in lua_text:
            unused.extend(paths); removed['particle'] += len(paths)
    for name, paths in cats['image'].items():
        if name not in lua_text:
            unused.extend(paths); removed['image'] += len(paths)
    for key, paths in cats['model'].items():
        if key + '.mdl' not in lua_text:
            unused.extend(paths); removed['model'] += len(paths)
...
def find_unused_materials(lua_text: str, mats: dict, keep_dirs: set):
    ...
    for _, paths in mats.items():
        for p in paths:
            ...
            if not any(rel.startswith(d) for d in keep_dirs) and key + '.vmt' not in lua_text and key + '.vtf' not in lua_text:
                unused.append(p); removed += 1
...
def main():
    ...
    print(f'{len(unused_non_mat)} non‑material assets totalling {humanize.naturalsize(size_non_mat)}')
    if unused_non_mat and input('Delete non‑material unused assets? (yes/no): ').strip().lower() == 'yes':
        freed = delete_paths(unused_non_mat)
        dirs_removed = clean_empty_dirs()
        for cat in ('sound', 'particle', 'image', 'model'):
            print(f'{removed_non_mat[cat]} {cat} files deleted')
        print(f'{dirs_removed} empty directories removed, freed {humanize.naturalsize(freed)}')
    entries, keep_dirs = extract_cdmaterials()
    unused_mat, removed_mat = find_unused_materials(lua_text, cats['material'], keep_dirs)
    size_mat = sum(os.path.getsize(p) for p in unused_mat)
    print(f'{len(unused_mat)} material assets totalling {humanize.naturalsize(size_mat)}')
    if unused_mat and input('Delete unused material assets? (yes/no): ').strip().lower() == 'yes':
