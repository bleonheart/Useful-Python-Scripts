...
def load_language_keys(path):
    ...
    keys = []
    seen = set()
    r_dot = re.compile(r'\bLANGUAGE\.(\w+)\s*=\s*("([^"\\]|\\.)*"|\'([^\'\\]|\\.)*\'|\[=*\[.*?\]=*\])', re.S)
    r_idx = re.compile(r'\bLANGUAGE\[\s*(["\'])(.*?)\1\s*\]\s*=\s*("([^"\\]|\\.)*"|\'([^\'\\]|\\.)*\'|\[=*\[.*?\]=*\])', re.S)
    ...
    while True:
        ...
        if body is None:
            pos = m.end()
            continue
        r_tbl1 = re.compile(r'(\w+)\s*=\s*("([^"\\]|\\.)*"|\'([^\'\\]|\\.)*\'|\[=*\[.*?\]=*\])', re.S)
        r_tbl2 = re.compile(r'\[\s*(["\'])(.*?)\1\s*\]\s*=\s*("([^"\\]|\\.)*"|\'([^\'\\]|\\.)*\'|\[=*\[.*?\]=*\])', re.S)
...
def count_placeholders(fmt):
    if not fmt:
        return 0
    return len(re.findall(r'%(?!%)(?:\d+\$)?[+\- #0]*?(?:\d+|\*)?(?:\.(?:\d+|\*))?(?:[hlL]?)[cdiouxXeEfgGqsa]', fmt))
...
def analyze_data(language_file, scan_dir):
    ...
    undefined, mismatches, all_used_keys = scan_localization_calls(scan_dir, language_file, keys, placeholders)
    undefined_rows = [(relpath(pth, scan_dir), ln, col, kind, key) for pth, ln, col, key, kind in undefined]
    undefined_rows.sort(key=lambda r: (r[0].lower(), r[1], r[2], r[3], r[4].lower()))
    mismatch_rows = [(relpath(pth, scan_dir), ln, col, kind, key, exp, got) for pth, ln, col, key, exp, got, kind in mismatches]
...
def write_framework_md(f, data, limit):
    write_markdown_table(f, ['Language file', 'Framework dir'], [[md_code(data['language_file']), md_code(data['scan_dir'])]])
    write_markdown_table(f, ['Metric', 'Value'], [
    ...
    if not data['mismatch_rows']:
        ...
        write_markdown_table(f, ['File', 'Line:Col', 'Call', 'Key', 'Expected args', 'Got args'], rows)
    ...
    if not data['undefined_key_names']:
        ...
        write_markdown_table(f, ['Key'], [[md_code(k)] for k in data['undefined_key_names'][:limit]])
...
def write_framework_txt(f, data, limit):
    ...
    f.write(f'Keys used but not defined:           {len(data["undefined_key_names"])}\n')
    ...
    if data['mismatch_rows']:
        write_header(f, ['file', 'line', 'col', 'call', 'key', 'expected_args', 'got_args'])
        for r in data['mismatch_rows'][:limit]:
...
def write_modules_md(f, modules, limit, modules_root):
    ...
    write_markdown_table(f, ['Module', 'Path', 'Missing (total)', 'Provided by framework', 'Truly missing'], summary)
    for m in mods:
        f.write(f'#### {m["name"]}\n\n')
        ...
        if missing_new:
            ...
            write_markdown_table(f, ['Key'], [[md_code(k)] for k in missing_new[:limit]])
    ...
    conflict_keys = [k for k, lst in key_to_defs.items() if len(lst) > 1 and len({v for _, v in lst}) > 1]
...
def write_modules_txt(f, modules, limit, modules_root):
    ...
    f.write('Modules Summary\n')
    f.write('---------------\n')
    write_header(f, ['module', 'path', 'missing_total', 'provided_by_framework', 'truly_missing'])
    for m in mods:
        total_missing = len(m['missing'])
        covered = len(m.get('covered_by_framework', []))
        truly_missing = len(m.get('missing_not_in_framework', []))
        f.write(f'{m["name"]}\t{relpath(m["module_dir"], modules_root)}\t{total_missing}\t{covered}\t{truly_missing}\n')
    f.write('\n')
    for m in mods:
        f.write(f'{m["name"]}\n')
        f.write('-' * len(m["name"]) + '\n')
    ...
    if not duplicate_keys:
        ...
        f.write(f'Duplicate keys (same ID in multiple modules)\t{len(duplicate_keys)}\n')
        f.write(f'Conflicting keys (same ID, different values)\t{len(conflict_keys)}\n\n')
        if conflict_keys:
            f.write('Conflicting keys (different values)\n')
...
def delete_standalone_assignments(src, delete_keys):
    dot = re.compile(r'(^[ \t]*LANGUAGE\.(\w+)\s*=\s*(?:"([^"\\]|\\.)*"|\'([^\'\\]|\\.)*\'|\[=*\[.*?\]=*\])[ \t]*\r?\n?)', re.M | re.S)
    idx = re.compile(r'(^[ \t]*LANGUAGE\[\s*(["\'])(.*?)\2\s*\]\s*=\s*(?:"([^"\\]|\\.)*"|\'([^\'\\]|\\.)*\'|\[=*\[.*?\]=*\])[ \t]*\r?\n?)', re.M | re.S)
    spans = []
    removed = 0
...
def main():
    ...
    if not os.path.isdir(a.framework_gamemode_dir):
        print('Framework dir not found', file=sys.stderr); sys.exit(1)
    if not os.path.isdir(a.framework_languages_dir):
        print('Framework languages dir not found', file=sys.stderr); sys.exit(1)
    if not os.path.isdir(a.modules_root):
        print('Modules root not found', file=sys.stderr); sys.exit(1)
    names = [f for f in os.listdir(a.framework_languages_dir) if f.lower().endswith('.lua')]
    if not names:
        print('No framework language files found', file=sys.stderr); sys.exit(1)
    fmt = a.format
    if fmt == 'auto':
    ...
    if any_unused:
        ...
        if ans in ('y', 'yes'):
            for lang, framework in framework_results:
                if framework['unused']:
                    replaced, removed = cleanup_language_file(framework['language_file'], framework['unused'], framework['lang_map'])
                    print(f'{lang}: framework {os.path.basename(framework["language_file"])} cleaned ({replaced} table(s) rebuilt, {removed} standalone removed)')
            for lang, modules in modules_results:
                for m in modules:
                    if m['unused']:
                        replaced, removed = cleanup_language_file(m['language_file'], m['unused'], m['lang_map'])
                        print(f'{lang}: module {m["name"]} cleaned ({replaced} table(s) rebuilt, {removed} standalone removed)')
        else:
